<template>
  ...
  const editQuestion = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/questions/${route.params.id}`, {
  ...
  const fetchQuestion = async () => {
    try {
      ...
      date.value = question.date.split('T')[0] // Ensure the date is in the correct format for the input field
    } catch (error) {
